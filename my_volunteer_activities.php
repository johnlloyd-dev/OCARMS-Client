                ...
                <div class="row mt-5">
                    <div class="col-lg-12">
                        <ul class="nav nav-tabs">
                            ...
                            <li class="nav-item font-weight-bold">
                                <a class="nav-link active items" href="my_volunteer_activities.php">My Volunteer
                                    Activities</a>
                            </li>
                            <li class="nav-item font-weight-bold">
                ...
                <div class="row mt-5 justify-content-center">
                    <div class="col-lg-12 mb-3">
                        <div class="card">
                            <div class="card-header">
                                <h5>
                                    <?=$user['client_name'];?>'s Pending Volunteer Application
                                </h5>
                            </div>
                            <div class="card-body">
                                <div class="table-wrap-2">
                                    <table class="table-2" id="table-2">
                                        ...
                                        <?php
                                    require 'includes/connect.php';
                                    $get_info = mysqli_query($conn, "SELECT * FROM `volunteer_info` WHERE `client_id`='$client_id' AND `vl_status`= 'Confirmed/Pending'");
                                    while($get = mysqli_fetch_assoc($get_info)){
                                    $event_id = $get['event_id'];
                                    $get_info2 = mysqli_query($conn, "SELECT * FROM `volunteer_event` WHERE `ve_id`='$event_id'");
                                    $get2 = mysqli_fetch_assoc($get_info2);
                                ?>
                                        <tbody>
                                            <tr>
                                                <th scope="row" class="scope">
                                                    <?=date("F j, Y", strtotime($get['vl_signup_date'])); echo "/"; echo date("g:i A", strtotime($get['vl_signup_time']))?>
                                                </th>
                                                <td><?=$get2['ve_name'];?></td>
                                                <td><?=date("F j, Y", strtotime($get2['ve_date']));?></td>
                                                <td>
                                                    <button type="button" class="btn btn-sm btn-warning" data-toggle="modal" data-target="#pendingModal<?=$get['vl_id']?>"><i class="fas text-white fa-info-circle"></i></button>
                                                    <a href="#cancel_volunteer<?=$get['vl_id']?>" class="btn btn-primary btn-sm" data-toggle="modal">Cancel</a>
                                                </td>
                                            </tr>
                                        </tbody>
                                        <?php 
                                        include('includes/pending_activity_modal.php');
                                        include('includes/volunteer_activities/cancel_volunteer.php');
                                    } 
                                    ?>
                                    </table>
                                </div>
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-header">
                                <h5>
                                    <?=$user['client_name'];?>'s Attended/Cancelled Volunteer Activities
                                </h5>
                            </div>
                            <div class="card-body">
                                <div class="table-wrap-2">
                                ...
                                ?>
                                        <tbody>
                                            <tr>
                                                ...
                                                <td><?=date("F j, Y", strtotime($get2['ve_date']));?></td>
                                                <td>
                                                    <button type="button" class="btn btn-sm btn-warning" data-toggle="modal" data-target="#atcaModal<?=$get['vl_id']?>"><i class="fas text-white fa-info-circle"></i></button>
                                                </td>
                                            </tr>
                                        </tbody>
                                        <?php 
                                        include('includes/att_can_activity_modal.php');
                                    } 
                                    ?>
                                    </table>
                                </div>
